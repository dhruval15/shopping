...
let headername = sessionStorage.getItem("userdetails") != null ? sessionStorage.getItem("userdetails") : 'Login';
...
var header = `  <header class=" bg-white container-fluid s-pad">
<nav class="navbar navbar-expand-lg  d-flex justify-content-between ">
    <div class="container-fluid ">
        <div class="d-flex">
            <img class="img1 bg-transparent pe-3" src="shopping.jpeg">
        </div>
        <button id="down" class="navbar-toggler down position-absolute order-3 border-0 bg-light ms-3 end-0 text-white">
                ...
                <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                    ...
                    <li class="nav-item item-drop py-4 dropdown pe-4 position-relative">
                        ...
                        <ul class="dropdown-menu position-absolute m-0"
                            ...
                            <li><a class="dropdown-item redirect" data-category="laptops"
                                    href="category.html?category=laptops">Laptops</a></li>

                            <li><a class="dropdown-item redirect" data-category="fragrances"
                                    href="category.html?category=fragrances">Fragrances</a>
                            </li>
                            <li><a class="dropdown-item redirect" data-category="skincare"
                                    href="category.html?category=skincare">Skincare</a>
                            </li>
                            <li><a class="dropdown-item redirect" data-category="groceries"
                                    href="category.html?category=groceries">Groceries</a>
                            </li>
                        </ul>
                    </li>
                    ...
                    <li class="nav-item item-drop dropdown pe-4 py-4">
                        ...
                        <ul class="dropdown-menu" style="top: 86px;border-top: 3px solid #9F78FF;">
                            <li><a class="dropdown-item redirect" data-brand="Apple"
                                    href="brand.html?brand=Apple">Apple</a>
                            </li>
                            <li><a class="dropdown-item redirect" data-brand="OPPO"
                                    href="brand.html?brand=oppo">OPPO</a>
                            </li>
                            <li><a class="dropdown-item redirect" data-brand="Samsung"
                                    href="brand.html?brand=smasung">Samsung</a></li>
                            <li><a class="dropdown-item redirect" data-brand="Huawei"
                                    href="brand.html?brand=huawei">Huawei</a>
                            </li>
                            ...
                            <li><a class="dropdown-item redirect" data-brand="Dry Rose"
                                    href="brand.html?brand=dry Rose">Dry
                                    Rose</a>
                            </li>
                            <li><a class="dropdown-item redirect " data-brand="Flying Wooden"
                                    href="brand.html?brand=Flying Wooden">Flying
                                    Wooden</a></li>
                            <li><a class="dropdown-item redirect" data-brand="Golden"
                                    href="brand.html?brand=golden">Golden</a>
                            </li>
                        </ul>
                    </li>
                    <li class="nav-item pe-4 py-4">
        ...
        <label class="px-1 text-dark showName">`+ headername.toUpperCase() + `</label>
            <button type="button" class="bg-transparent border-0 fa-solid fa-user ps-1 fa-21x" data-bs-toggle="modal"
            data-bs-target="#exampleModal" style="color:cornflowerblue;"></button>
        </div>
    </div>
</nav>
</div>
...
<div class="modal fade " id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" >
            <div class="modal-dialog ">
                <div class="modal-content loginTitle bg-white">
                    <div class="modal-header justify-content-center border-0">
                        <h1 class="about modal-title fs-7 text-primary" id="exampleModalLabel">Login</h1>
                    </div>
                    <form id="login-form">
                    <div class="modal-body">
                            <div class="mb-3">
                                <label for="recipient-name" class="col-form-label">Username:</label>
                                <input type="text" name="username" class="form-control" id="recipient-name" placeholder="Type your username">
                                <span id="usercheck" style="color: red;display: none">
                                    **Username is missing
                                </span>
                            </div>
                            <div class="mb-4">
                                <label for="message-text" class="col-form-label">Password:</label>
                                <input type="password" name="password" class="form-control" id="message-text" placeholder="Type your password">
                                <span id="passcheck" style="color: red;display: none">
                            ...
                            <!-- Submit button -->
                            <button type="submit" class="w-100 submit btn btn-primary btn-block mb-4"  >Sign in</button>
...
var add = `<nav aria-label="breadcrumb">
<ol class="breadcrumb">
...
function validateUsername() {
    ...
    let usernameValue = $("#recipient-name").val();
...
function validatePassword() {
    console.log('called');
    let passwordValue = $("#message-text").val();
...
let cartError = document.getElementsByClassName('showError');
console.log(cartError.innerHTML);
// $('.showError').html("hello")

    if (cartProducts.length === 0) {
        // console.log('error');
        $('.showError').text('cart is empty');
    }
...
$('.submit').click(function () {
    ...
    let password = document.getElementById('message-text');
    if (username.value && password.value === "") {
        console.log('wrong');
    } else {
        sessionStorage.setItem('userdetails', username.value);
        sessionStorage.setItem('passwordData', password.value);
        $(".showName").text(username.value.toUpperCase());
    }
})
$(".showName").text(headername.toUpperCase());
...
function displayProds(products, isWishList) {
    var store = JSON.parse(sessionStorage.getItem('wishlistProducts'));
    $(".datapush").empty();
    for (var i = 0; i < products.length; i++) {
        ...
        let car = '<div class="col-sm-12 col-md-6 col-lg-4 g-3 radius">'
        car += '<div class="card border-0 showFunction position-relative object-fit">'
        car += '<img class="pic card-img-top position-relative object-fit" src=' + product.thumbnail + ' />'
        car += ' <p class="position-absolute imgPrice"> ₹ ' + product.price + '</p>'
        car += '<div style="text-align: center" class="position-absolute stars">'
        car += '<div class="stars-outer">'
        car += '<div class=" stars-inner"  style="width:' + starPercentageRounded + '%;">'
        car += '</div></div></div>'
        car += `<div class="position-absolute imgFunction bg-light d-flex justify-content-around"><button class=" cart1 border-0 " onclick="pushCart(${product.id})" data-product-id=${product.id} `
        car += '><i class="fa-solid live fa-cart-shopping fs-5"></i></button>'
        car += '<span class="live"> ' + product.title + '</span>'
        car += `<button class="wishlist border-0 text-color-blue " onclick="handleWishList(${product.id},${isWishList})" data-product-id=${product.id} >`
        car += '<i class="fa-regular fa-heart live fs-5"></i>'
        car += '</button></div>'
        car += '</div></div></div>'
        $('.datapush').append(car);
    }
